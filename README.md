# Store System🏪

A Store System é uma loja com foco em cadastramento de produtos com scanner que o usuário deve iniciar para começar a cadastrar as opções que são aceitadas, estão armazenadas dentro de um Enum.

Futuramente, quando eu dominar o banco de dados, eu vou atualizar o projeto. Ele é bem simples, mas funcional.

Como utilizar?

Digite a opção que está salva no Enum que você pode ver nos utils ou só interprete o texto e, com base nas perguntas, você dá uma entrada no produto.
